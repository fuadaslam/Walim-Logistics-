...
class AttendanceState {
  ...
  AttendanceState({this.isCheckingIn = false, this.hasActiveShift = false, this.error});

  AttendanceState copyWith({bool? isCheckingIn, bool? hasActiveShift, String? error}) {
...
class AttendanceNotifier extends StateNotifier<AttendanceState> {
  ...
  Future<void> _checkActiveShift() async {
    ...
    if (profile != null) {
      ...
      state = state.copyWith(hasActiveShift: activeShift != null);
    }
  }
  ...
  Future<void> toggleShift({
    ...
    try {
      ...
      if (state.hasActiveShift) {
        ...
        if (!isValid) {
          state = state.copyWith(error: 'You are outside the geofenced area!', hasActiveShift: true, isCheckingIn: false);
        } else {
          state = state.copyWith(hasActiveShift: true, isCheckingIn: false);
        }
      }
    } catch (e) {
